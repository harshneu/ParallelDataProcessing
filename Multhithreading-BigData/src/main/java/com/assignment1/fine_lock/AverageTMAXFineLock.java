...
public class AverageTMAXFineLock {
	...
	public void fineCalculation(List<List<String>> divisions) throws Exception{
		Thread thread1 = new Thread(new ThreadFineLock(divisions.get(0)));
		Thread thread2 = new Thread(new ThreadFineLock(divisions.get(1)));
		Thread thread3 = new Thread(new ThreadFineLock(divisions.get(2)));
		Thread thread4 = new Thread(new ThreadFineLock(divisions.get(3)));
		//start the thread
		thread1.start();
		thread2.start();
		thread3.start();
