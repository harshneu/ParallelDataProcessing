...
public class ThreadsStart {
	...
	public void coarseLockCalculation(List<List<String>> divisions) throws Exception{
		Thread thread1 = new Thread(new ThreadCoarseLock(divisions.get(0)));
		Thread thread2 = new Thread(new ThreadCoarseLock(divisions.get(1)));
		Thread thread3 = new Thread(new ThreadCoarseLock(divisions.get(2)));
		Thread thread4 = new Thread(new ThreadCoarseLock(divisions.get(3)));
