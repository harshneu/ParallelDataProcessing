...
public class AverageTMAXNoLock {
	...
	public void noLockCalculation(List<List<String>> divisions) throws Exception{
		Thread thread1 = new Thread(new ThreadNoLock(divisions.get(0)));
		Thread thread2 = new Thread(new ThreadNoLock(divisions.get(1)));
		Thread thread3 = new Thread(new ThreadNoLock(divisions.get(2)));
		Thread thread4 = new Thread(new ThreadNoLock(divisions.get(3)));
		//start the thread
		thread1.start();
		thread2.start();
		thread3.start();
