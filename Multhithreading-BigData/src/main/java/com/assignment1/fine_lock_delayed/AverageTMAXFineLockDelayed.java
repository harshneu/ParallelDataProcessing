...
public class AverageTMAXFineLockDelayed {

	// The thread with the station holds the lock.
	public static HashMap<String, Station> stations = new HashMap<String, Station>();

	public void fineCalculation(List<List<String>> divisions) throws Exception {
		Thread thread1 = new Thread(new DelayThreadFineLock(divisions.get(0)));
		Thread thread2 = new Thread(new DelayThreadFineLock(divisions.get(1)));
		Thread thread3 = new Thread(new DelayThreadFineLock(divisions.get(2)));
		Thread thread4 = new Thread(new DelayThreadFineLock(divisions.get(3)));
